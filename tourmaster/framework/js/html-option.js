	...
	function tourmaster_bind_admin_rating(){
		...
		if( admin_review.length ){
			...
			admin_review.on('click', '.tourmaster-single-review-edit', function(){
				...
				if( item_holder.attr('data-room-id') ){
					sent_data['room_id'] = item_holder.attr('data-room-id');
				}else if( item_holder.attr('data-tour-id') ){
					sent_data['tour_id'] = item_holder.attr('data-tour-id'); 
				}
		...
		get_val: function(){
			...
			this.container.find('[data-slug]').each(function(){
				...
				if( input_type == 'text' || input_type == 'textarea' || input_type == 'combobox' ||  
					...
					if( typeof($(this).data('value')) == 'object' && $(this).data('value').length == 0 ){
						obj[slug] = "";
					}else{
	...
	tourmaster_tabs.prototype = {
		...
		init: function(){
			...
			t.container.on('click', '.tourmaster-html-option-tabs-duplicate', function(){
				...
				var inner_tab = clone.find('.tourmaster-html-option-tabs-field-tab');
				if( inner_tab.length ){
					var inner_tab_clone = inner_tab.clone(false);
					inner_tab_clone.children().remove();
					inner_tab_clone.data('value', inner_tab.data('value'));
					inner_tab_clone.data('options', inner_tab.data('options'));
					inner_tab_clone.data('settings', inner_tab.data('settings'));
					inner_tab.replaceWith(inner_tab_clone);

					new tourmaster_tabs(inner_tab_clone);
				}
