(function($){
	...
	// tourmaster lightbox
	function tourmaster_lightbox_room_edit( lightbox_wrap ){
		
		lightbox_wrap.on('change', '[name="room_amount"]', function(){
			var room_amount = $(this).val();
			var templates = $(this).closest('.tourmaster-order-edit-field').siblings('.tourmaster-room-order-add-guests');
			if( room_amount > templates.length ){
				for( var i = 0; i < room_amount - templates.length; i++ ){
					var clone = templates.first().clone(false);
					clone.css('display', 'none');
					clone.find('.tourmaster-room-order-add-guests-label span').html(room_amount - i);
					clone.insertAfter(templates.last());
					clone.slideDown(150);
				}
			}else if( room_amount < templates.length ){
		...
		lightbox_wrap.on('click', '.tourmaster-room-admin-remove-order', function(){
			...
			tourmaster_confirm_box({
				...
				success: function(){
					...
					remove_button.closest('.tourmaster-my-booking-single-field').slideUp(150, function(){ $(this).remove() });
					tourmaster_tour_booking_ajax(form.attr('data-ajax-url'), {
						success: function(data){
							console.log(data);
							if( data.status == 'success' ){
								form.find('[name="booking_details"]').val(JSON.stringify(data.booking_details));
								form.find('.tourmaster-new-order-booking-detail-data').html(data.booking_details_data);
							}
						}
					}, {
						action: 'tourmaster_room_admin_remove_order',
						data: sent_data, 
						index: index_val
					});
		...
		lightbox_wrap.on('click', '.tourmaster-room-add-to-order', function(){
			...
			form.find('.tourmaster-order-edit-form-error').slideUp(150, function(){ $(this).remove(); });
			tourmaster_tour_booking_ajax(form.attr('data-ajax-url'), {
				success: function(data){
					button.removeClass('tourmaster-now-loading');

					if( data.status == 'failed' ){
						var error_message = $('<div class="tourmaster-order-edit-form-error" ></div>').html(data.message);
						error_message.insertBefore(button);
						error_message.slideDown(150);
					}else{
						form.find('[name="tid"]').val(data.tid);
						form.find('[name="booking_details"]').val(JSON.stringify(data.booking_details));
						form.find('.tourmaster-new-order-booking-detail-data').html(data.booking_details_data);
					}
				}
			}, {
				action: 'tourmaster_room_admin_add_order',
				data: sent_data
			});
	...
	function tourmaster_lightbox( content ){
		...
		// datepicker
		lightbox_wrap.find('.tourmaster-datepicker').datepicker({
			dateFormat: 'yy-mm-dd',
			changeMonth: true,
			changeYear: true
		});
		...
		// room edit action
		tourmaster_lightbox_room_edit(lightbox_wrap);

		// tour edit action
		tourmaster_lightbox_tour_edit(lightbox_wrap);

		// billing & contact action
		$('.tourmaster-payment-billing-separate').each(function(){ 
			if( $(this).is(':checked') ){
				$(this).closest('.tourmaster-payment-billing-separate-wrap').siblings('.tourmaster-room-payment-billing-wrap').show();
			}

			$(this).on('change', function(){
				if( $(this).is(':checked') ){
					$(this).closest('.tourmaster-payment-billing-separate-wrap').siblings('.tourmaster-room-payment-billing-wrap').slideDown(150);
				}else{
					$(this).closest('.tourmaster-payment-billing-separate-wrap').siblings('.tourmaster-room-payment-billing-wrap').slideUp(150);
				}
			});
		});	
	...
	$(document).ready(function(){
		...
		$('.tourmaster-datepicker').each(function(){
			if( $(this).closest('.tourmaster-lightbox-content').length ) return;
