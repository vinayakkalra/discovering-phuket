		...
		function tourmaster_mail_notification( $type, $tid = '', $user_id = '', $settings = array() ){
			...
			if( $option_enable == 'enable' ){
				...
				if( !empty($result) ){
					...
					$mail_title = strip_tags(str_replace('{tour-name}', $tour_name, $mail_title));
						...
						if( !empty($field_value) ){
							if( $field_key == 'submission-amount' && is_array($field_value) ){
								...
								if( !empty($field_value['amount']) || !empty($field_value['deposit_price']) ){
									...
									if( !empty($field_value['amount']) ){
										$temp_content .= '<span class="payment-method" >' . tourmaster_money_format($field_value['amount']) . '</span>';	
									}else{
										$temp_content .= '<span class="payment-method" >' . tourmaster_money_format($field_value['deposit_price']) . '</span>';	
									}
									$temp_content .= '</div>';
								}
							}else{
				...
				if( $type == 'admin-registration-complete-mail' || $type == 'registration-complete-mail' ){
					$is_tour_enable = tourmaster_get_option('general', 'enable-tour', 'enable');
					if( $is_tour_enable == 'disable' && function_exists('tourmaster_room_mail_content') ){
						$room_mail = true;
		...
		function tourmaster_send_email_invoice( $tid ){
			...
			if( !empty($result->traveller_info) ){
				...
				if( !empty($traveller_info) ){
					echo '<div style="margin-bottom: 50px; " >'; // tourmaster-invoice-single-traveller-info
					echo '<h3 style="font-size: 16px; text-transform: uppercase; margin-top: -20px; font-weight: 800" >' . esc_html__('Traveller Info', 'tourmaster') . '</h3>'; // tourmaster-invoice-single-title
					for( $i=0; $i<sizeof($traveller_info['first_name']); $i++ ){
						if( !empty($traveller_info['first_name'][$i]) || !empty($traveller_info['last_name'][$i]) ){
							echo '<div>'; // tourmaster-invoice-single-field
							echo '<span>' . esc_html__('Traveller', 'tourmaster') . ' ' . ($i+1) . ' :</span> '; // tourmaster-head
							echo '<span>'; // tourmaster-tail
			...
			if( !empty($result->order_status) && in_array($result->order_status, array('approved', 'online-paid', 'departed', 'deposit-paid')) ){
					...
					foreach( $payment_infos as $payment_info ){
						...
						if( !empty($payment_info['amount']) || !empty($payment_info['deposit_price']) ){
							...
							if( !empty($payment_info['amount']) ){
								...
								echo '<div>' . tourmaster_money_format($payment_info['deposit_price']) . '</div>';	// tourmaster-tail
							}
							echo '</div>'; // tourmaster-invoice-payment-info-item
						}
