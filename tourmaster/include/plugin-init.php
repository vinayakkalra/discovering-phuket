	...
	*/
	...
	if( !function_exists('tourmaster_table_init') ){
		function tourmaster_table_init(){
			...
			// room order table
			$sql = "CREATE TABLE {$wpdb->prefix}tourmaster_room_order (
				id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
				user_id bigint(20) UNSIGNED DEFAULT NULL,
				booking_date datetime DEFAULT '0000-00-00 00:00:00' NOT NULL,
			...
			// room order table
			$sql = "CREATE TABLE {$wpdb->prefix}tourmaster_room_booking (
				id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
				order_id bigint(20) UNSIGNED DEFAULT NULL,
			...
			// room review table
			$sql = "CREATE TABLE {$wpdb->prefix}tourmaster_room_review (
				review_id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
				order_id bigint(20) UNSIGNED DEFAULT NULL,
				review_room_id bigint(20) UNSIGNED NOT NULL,
				review_score tinyint DEFAULT NULL,
				review_description longtext DEFAULT NULL,
				review_date datetime DEFAULT '0000-00-00 00:00:00' NOT NULL,
				reviewer_name varchar(100) DEFAULT NULL,
	...
	if( !function_exists('tourmaster_set_plugin_role') ){
		function tourmaster_set_plugin_role(){
			...
			foreach( $post_type_cap as $cap ){
				...
				$admin->add_cap(str_replace('%s', 'room', $cap));
			...
			$admin->add_cap('manage_room_category');
			$admin->add_cap('manage_room_tag');
			$admin->add_cap('manage_room_filter');
			$admin->add_cap('manage_room_order');
	...
	add_filter('wp_dropdown_users_args', 'tourmaster_wp_dropdown_users_args', 10, 2);
	if( !function_exists('tourmaster_wp_dropdown_users_args') ){
		function tourmaster_wp_dropdown_users_args($query_args, $parse_args){
			if( $parse_args['who'] == 'authors' ){
